...
\begin{tabular}{p{\OptieBreed}p{\linewidth-\OptieBreed}}
	...
	\shell{user.name}		& Je naam, zichtbaar in alle commits. Verplicht!\\
	\shell{user.email}		& Email, zichtbaar in alle commits. Verplicht!\\
	\shell{core.editor} 	& Editor voor commit messages. Zet op \texttt{"{}atom --wait"} (Download Atom: \url{https://atom.io}).\\
...
\end{itemize}
Zie \shell{git help ignore} voor alle mogelijkheden met filters. Voorbeeldfilterbestand:
...
\begin{description}
	\item[git stash] Je wijzigingen tijdelijk `opzij' zetten met een handig commando (zie \shell{git help stash}, \shell{git
			stash pop} voor ongedaan).
	\item[De workshop] Hulp nodig? Join Discord \url{https://discord.gg/qvXvaam} , slides op
		\url{https://j.mp/gitgudslides}
	\item Git + Visual Studio: \url{https://visualstudio.github.com}, GUI-clients:
		\url{https://syntevo.com/smartgit}, \url{https://gitkraken.com}
...
{ \footnotesize
	...
	\begin{multicols}{4}
		\begin{itemize}
			...
			\item \shell{atom best} - Bewerk \shell{best}
			\item \shell{mv} - Verplaats / hernoem
			\item \shell{cp} - Kopi\"eer
			\item \shell{rm} - Verwijder bestand
